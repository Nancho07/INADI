        ...
        if(datos.success==true){
          ...
          usuario.aplicacion_parent_k = 6;
            if (typeof(Storage) !== "undefined") {
                localStorage.setItem("inputUrl", $("#inputrootUrl").val());
                localStorage.setItem("inputUsuario", $("#inputUsuario").val());
                localStorage.setItem("inputEmail", $("#inputEmail").val());
            } else {
                var mensaje = {
                    mensaje:"No puede almacenar datos locales, los datos de acceso debera escribirlos cada vez que entre a la aplicación",
                    success:false
                };
                delServidor(mensaje);
            }
...
function onAppReady(){
    if (typeof(Storage) !== "undefined") {
        ...
        var mensaje = {
            mensaje:"No hay registros de ingreso",
            success:false
        };
        delServidor(mensaje);
    }
